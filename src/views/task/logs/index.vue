  ...
  <div>
    <el-card shadow="always">
      <el-form ref="searchForm"
        ...
        <el-form-item label="任务名称" prop="jobName">
          <el-input type="text" v-model="pageInfo.jobName" placeholder="请输入关键字"/>
        </el-form-item>
        <el-form-item>
      ...
      <el-table border :data="data" :loading="loading">
        ...
        <el-table-column prop="status" label="执行结果" width="200">
          <template slot-scope="scope">
            <el-badge is-dot v-if="scope.row.status===1" type="success" text="成功"/>
            <el-badge is-dot v-else="" type="danger" text="失败"/>
    ...
    <el-drawer size="40%" :with-header="false" :visible.sync="drawer">
      <div style="padding-left:16px;padding:8px;border-bottom:1px solid  #ccc">
        <el-badge is-dot v-if="currentRow.status===1" type="success"/>
      ...
      <div style="padding-left:16px;margin-bottom:40px">
        ...
        <h5 style="color:#707070" v-if="currentRow.cronExpression">cron表达式：{{currentRow.cronExpression}}</h5>
        <h5 style="color:#707070" v-else="">调度时间：: {{currentRow.startDate}} ~ {{currentRow.endDate}}</h5>
