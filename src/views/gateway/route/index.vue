  ...
  <div>
    <el-card shadow="always">
      <div class="search-con search-con-top">
        <el-button-group>
          <el-button :disabled="hasAuthority('gatewayRouteEdit')?false:true" class="search-btn" type="primary"
                  @click="handleModal()">
            <span>添加</span>
          </el-button>
        </el-button-group>
        ...
        <el-table-column label="操作" fixed="right" width="200">
          <template slot-scope="scope">
            <a class="linkfont" :disabled="hasAuthority('gatewayRouteEdit')?false:true" @click="handleModal(scope.row)">
              编辑</a>&nbsp;
            <el-dropdown v-show="hasAuthority('gatewayRouteEdit')" transfer ref="dropdown"
                      @command="handleClick">
      ...
      <div>
        <el-form ref="routeForm" :model="formItem" :rules="formItemRules" label-width="100px">
          <el-form-item label="路由名称" prop="routeDesc">
            <el-input v-model="formItem.routeDesc" placeholder="请输入内容"></el-input>
          </el-form-item>
          <el-form-item label="路由标识" prop="routeName">
            <el-input v-model="formItem.routeName" placeholder="默认使用服务名称{application.name}"></el-input>
          </el-form-item>
          <el-form-item label="路由前缀" prop="path">
            <el-input v-model="formItem.path" placeholder="/{path}/**"></el-input>
          </el-form-item>
          <el-form-item label="路由方式">
            <el-select v-model="selectType">
              <el-option value="service" label="负载均衡(serviceId)"></el-option>
              <el-option value="url" label="反向代理(url)"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="selectType==='service'" label="负载均衡" prop="serviceId"
                    :rules="{required: true, message: '服务名称不能为空', trigger: 'blur'}">
            <el-input v-model="formItem.serviceId" placeholder="服务名称application.name"></el-input>
          </el-form-item>
          <el-form-item v-if="selectType==='url'" label="反向代理" prop="url"
                    :rules="[{required: true, message: '服务地址不能为空', trigger: 'blur'},{type: 'url', message: '请输入有效网址', trigger: 'blur'}]">
            <el-input v-model="formItem.url" placeholder="http://localhost:8080"></el-input>
          </el-form-item>
