    ...
    <el-card shadow="always">
      ...
      <el-table border :data="data" :loading="loading">
        ...
        <el-table-column prop="policyType" label="策略类型" width="200">
          <template slot="policyType" slot-scope="{ row }">
            <el-tag type="success" v-if="row.policyType===1">允许-白名单</el-tag>
            <el-tag type="danger" v-else="">拒绝-黑名单</el-tag>
          </template>
        </el-table-column>
      ...
      <el-col :span="24" class="toolbar">
        <el-pagination background @size-change="handlePageSize" @current-change="handlePage"
          :current-page="pageInfo.page" :page-sizes="[10, 20, 50, 100]" :page-size="pageInfo.limit"
      ...
      <div>
        <el-tabs v-model="current" @tab-click="handleTabClick">
          <el-tab-pane label="策略信息" name="form1">
            <el-form ref="form1" v-show="current=='form1'" :model="formItem" :rules="formItemRules" label-width="100px">
              <el-form-item label="策略名称" prop="policyName">
                <el-input v-model="formItem.policyName" placeholder="请输入内容"></el-input>
              </el-form-item>
              <el-form-item label="策略类型" prop="policyType">
                <el-select v-model="formItem.policyType">
                  <el-option value="url" label="接口(url)"></el-option>
                  <el-option disabled value="origin" label="来源(origin)"></el-option>
                  <el-option disabled value="user" label="用户(user)"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="单位时间" prop="intervalUnit">
                <el-select v-model="formItem.intervalUnit">
                  <el-option value="seconds" label="秒(seconds)"></el-option>
                  <el-option value="minutes" label="分钟(minutes)"></el-option>
                  <el-option value="hours" label="小时(hours)"></el-option>
                  <el-option value="days" label="天(days)"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="限流数" prop="limitQuota">
                <el-input-number :min="10" v-model="formItem.limitQuota"></el-input-number>
              </el-form-item>
            </el-form>
          </el-tab-pane>
            ...
            <el-form ref="form2" v-show="current=='form2'" :model="formItem" :rules="formItemRules">
              <el-form-item prop="authorities">
                <el-transfer :data="selectApis" :titles="['选择接口', '已选择接口']" :render-content="transferRender"
                  v-model="formItem.apiIds" @change="handleTransferChange" filterable></el-transfer>
              </el-form-item>
            </el-form>
          </el-tab-pane>
        </el-tabs>
